...
```bash
bash scripts/sweep.sh --wallet 0xYOUR_WALLET --threshold 0.001
```
...
```text
"What dust tokens are in my wallet 0xabc...? Are any worth transferring?"
```

The agent will run `bash scripts/sweep.sh --demo` (or the live command with the address you gave) and read the result back to you.
...
pharos-skill install https://github.com/cexco10/dustcleaner
```

**Option B — OpenClaw / Claude Code / Codex** (one-line via npm):

```bash
npx skills add https://github.com/cexco10/dustcleaner
```
...
mkdir -p ~/.claude/skills/dustcleaner
cp -r . ~/.claude/skills/dustcleaner/

# Codex: copy to ~/.codex/skills/
mkdir -p ~/.codex/skills/dustcleaner
cp -r . ~/.codex/skills/dustcleaner/

# OpenClaw: copy to ~/.openclaw/skills/
mkdir -p ~/.openclaw/skills/dustcleaner
cp -r . ~/.openclaw/skills/dustcleaner/
